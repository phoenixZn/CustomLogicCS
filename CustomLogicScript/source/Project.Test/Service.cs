namespace HotUpdate.CoreGame
{
    ...
    public class CustomLogicService : ICustomLogicService
    {
        ...
        public CustomLogic CreateLogic(ICustomLogicGenInfo genInfo)
        {
            if (genInfo == null)
            {
                LogWrapper.LogError("CreateLogic genInfo == null");
                return null;
            }
        ...
        public T CreateLogic<T>(ICustomLogicGenInfo genInfo) where T : CustomLogic
        {
            if (genInfo == null)
            {
                LogWrapper.LogError($"CreateLogic<T> genInfo == null, T = {typeof(T)}");
                return null;
            }
            ...
            LogWrapper.LogError($"CreateLogic logic = {genInfo.LogicConfigID} logic({logic.GetType()}) is not {typeof(T)}");
            return null;
        }
