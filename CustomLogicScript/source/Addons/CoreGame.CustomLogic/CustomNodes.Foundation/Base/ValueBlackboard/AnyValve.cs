...
namespace HotUpdate.CoreGame
{
    public readonly struct AnyValve
    {
        ...
        public int AsInt =>
            IsInt ? (int)_long0 : throw new InvalidOperationException($"Cannot return as int _index={_index}");

        public long AsLong =>
            IsLong ? _long0 : throw new InvalidOperationException($"Cannot return as long _index={_index}");

        public bool AsBool =>
            IsBool ? _long0 != 0 : throw new InvalidOperationException($"Cannot return as bool _index={_index}");

        public float AsFloat =>
            IsFloat ? _float0 : throw new InvalidOperationException($"Cannot return as float _index={_index}");

        public Vector3 AsVector3 =>
            IsVector3 ? new Vector3(_float0, _float1, _float2) : throw new InvalidOperationException($"Cannot return as Vec3 _index={_index}");

        public Vector2 AsVector2 =>
            IsVector2 ? new Vector2(_float0, _float1) : throw new InvalidOperationException($"Cannot return as Vec2 _index={_index}");

        public Quaternion AsQuaternion =>
            IsQuaternion ? new Quaternion(_float0, _float1, _float2, _float3) : throw new InvalidOperationException($"Cannot return as Quaternion _index={_index}");

        public string AsString =>
            IsString ? _object0 as string : throw new InvalidOperationException($"Cannot return as String _index={_index}");

        public object AsObject =>
            IsObject ? _object0 : throw new InvalidOperationException($"Cannot return as Object _index={_index}");
        ...
        bool Equals(AnyValve other) =>
            ...
            _index switch
            {
                ...
                IndexVec3 => (Math.Abs((double)_float0 - (double)other._float0) < TOLERANCE) &&
                             (Math.Abs((double)_float1 - (double)other._float1) < TOLERANCE) &&
                             (Math.Abs((double)_float2 - (double)other._float2) < TOLERANCE),
                IndexVec2 => (Math.Abs((double)_float0 - (double)other._float0) < TOLERANCE) &&
                             (Math.Abs((double)_float1 - (double)other._float1) < TOLERANCE),
                IndexQuaternion => (Math.Abs((double)_float0 - (double)other._float0) < TOLERANCE) &&
                                   (Math.Abs((double)_float1 - (double)other._float1) < TOLERANCE) &&
                                   (Math.Abs((double)_float2 - (double)other._float2) < TOLERANCE) &&
                                   (Math.Abs((double)_float3 - (double)other._float3) < TOLERANCE),
